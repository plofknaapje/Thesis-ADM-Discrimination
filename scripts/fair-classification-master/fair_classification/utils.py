...
def train_model(data_package: dict, settings: dict):
    """
    ...
    Inputs:
        data_package:
            x_train: (n) x (d+1) numpy array -- n = examples, d = features, one feature is the intercept
            y_train: 1-d numpy array (n entries)
            x_control: dictionary of the type {"s": [...]}, key "s" is the sensitive feature name,
                and the value is a 1-d list with n elements holding the sensitive feature values

        settings:
            loss_function: the loss function that we want to optimize -- for now we have logistic loss,
                but other functions like hinge loss can also be added
            fairness: optimize accuracy subject to fairness constraint (0/1 values)
            accuracy: optimize fairness subject to accuracy constraint (0/1 values)
            ...
            gamma: controls the loss in accuracy we are willing to incur when using accuracy and sep_constraint
    ...
    """
    assert(not (settings["accuracy"] == 1 and settings["fairness"] == 1))

    x_train = data_package["x_train"]
    y_train = data_package["y_train"]
    x_control_train = data_package["x_control_train"]

    max_iter = 100000  # maximum number of iterations
    if settings["fairness"] == 0:
        constraints = []
    else:
        constraints = get_constraint_list_cov(data_package, settings)

    # its not the reverse problem, just train w with cross cov constraints
    if settings["accuracy"] == 0:
        f_args = (x_train, y_train)
        w = minimize(fun=settings["loss_function"],
        ...
        def constraint_gamma_all(w, x, y,  initial_loss_arr):
            ...
            new_loss = settings["loss_function"](w, x, y)
            old_loss = sum(initial_loss_arr)
            return ((1.0 + settings["gamma"]) * old_loss) - new_loss

        # dont confuse the protected here with the sensitive feature protected/non-protected values -- protected here
        # means that these points should not be misclassified to negative class
        ...
        def constraint_unprotected_people(w, ind, old_loss, x, y):
            ...
            new_loss = settings["loss_function"](w, np.array([x]), np.array(y))
            return ((1.0 + settings["gamma"]) * old_loss) - new_loss

        constraints = []
        predicted_labels = np.sign(np.dot(w.x, x_train.T))
        unconstrained_loss_arr = settings["loss_function"](
            w.x, x_train, y_train, return_arr=True)

        if settings["sep_constraint"]:  # separate gemma for different people
            for i in range(0, len(predicted_labels)):
                # for now we are assuming just one sensitive attr for reverse constraint
                if predicted_labels[i] == 1.0 and x_control_train[
                        settings["sensitive_attrs"][0]][i] == 1.0:
                    # this constraint makes sure that these people stay in the positive class
                    # even in the modified classifier
                    c = (
                        {'type': 'ineq', 'fun': constraint_protected_people,
                         'args': (x_train[i],
                                  y_train[i])})
                    constraints.append(c)
                else:
                    c = ({'type': 'ineq', 'fun': constraint_unprotected_people,
                          'args': (i, unconstrained_loss_arr[i],
                                   x_train[i], y_train[i])
                          })
                    constraints.append(c)
        else:  # same gamma for everyone
            c = ({'type': 'ineq', 'fun': constraint_gamma_all,
                 'args': (x_train, y_train, unconstrained_loss_arr)})
            constraints.append(c)
        ...
        w = minimize(
            fun=cross_cov_abs_optm_func, x0=old_w,
            args=(x_train, x_control_train[settings["sensitive_attrs"][0]]),
...
def compute_cross_validation_error(
        x_all, y_all, x_control_all, num_folds, loss_function,
        apply_fairness_constraints, apply_accuracy_constraint, sep_constraint,
        sensitive_attrs, sensitive_attrs_to_cov_thresh_arr, gamma=None):
    """
    Computes the cross validation error for the classifier subject to various fairness constraints
    This function is just a wrapper of "train_model(...)", all inputs (except for num_folds) are the same.
    sSee the specifications of train_model(...) for more info.

    Returns lists of train/test accuracy (with each list holding values for all folds),
    the fractions of various sensitive groups in positive class (for train and test sets),
    and covariance between sensitive feature and distance from decision boundary (again, for both train and test folds).
    """
    ...
    for i in range(0, num_folds):
        # shuffle the data before creating each fold
        ...
        x_train, y_train, x_control_train, x_test, y_test, x_control_test = split_into_train_test(
            x_all_perm, y_all_perm, x_control_all_perm, train_fold_size)

        train_folds.append([x_train, y_train, x_control_train])
        test_folds.append([x_test, y_test, x_control_test])

    def train_test_single_fold(
            train_data, test_data, fold_num, output_folds,
        ...
        w = train_model(
            x_train, y_train, x_control_train, loss_function,
            apply_fairness_constraints, apply_accuracy_constraint,
            sep_constraint, sensitive_attrs, sensitive_attrs_to_cov_thresh,
            gamma)
        train_score, test_score, _, _ = check_accuracy(
        ...
        correlation_dict_train = get_correlations(
            None, None, all_class_labels_assigned_train, x_control_train,
            sensitive_attrs)
        cov_dict_train = print_covariance_sensitive_attrs(
            None, x_train, distances_boundary_train, x_control_train,
            sensitive_attrs)

        output_folds.put([fold_num, test_score, train_score,
                          correlation_dict_test, correlation_dict_train,
                          cov_dict_test, cov_dict_train])
    ...
    processes = [
        Process(
            target=train_test_single_fold,
            args=(train_folds[x],
                  test_folds[x],
                  x, output_folds,
                  sensitive_attrs_to_cov_thresh_arr[x]))
    ...
    correlation_dict_train = []
    cov_dict_test = []
    cov_dict_train = []

    results = sorted(results, key=lambda x: x[0])  # sort w.r.t fold num
    for res in results:
        _, test_score, train_score, correlation_dict_test, correlation_dict_train, cov_dict_test, cov_dict_train = res

        test_acc.append(test_score)
        train_acc.append(train_score)
        correlation_dict_test.append(correlation_dict_test)
        correlation_dict_train.append(correlation_dict_train)
        cov_dict_test.append(cov_dict_test)
        cov_dict_train.append(cov_dict_train)

    return test_acc, train_acc, correlation_dict_test, correlation_dict_train, cov_dict_test, cov_dict_train
...
def print_classifier_fairness_stats(
    ...
    print("Protected/non-protected in +ve class: %0.0f%% / %0.0f%%" %
          (prot_pos, non_prot_pos))
...
def compute_p_rule(x_control, class_labels):
    ...
    non_prot_pos = sum(class_labels[x_control == 1.0] == 1.0)
    # protected in positive class
    prot_pos = sum(class_labels[x_control == 0.0] == 1.0)
    ...
    print("Non-protected in positive class: %d (%0.0f%%)" %
          (non_prot_pos, non_prot_pos * 100.0 / non_prot_all))
    print("Protected in positive class: %d (%0.0f%%)" %
...
def check_accuracy(model, data_package, y_train_predicted, y_test_predicted):
    """
    ...
    if model is not None:
        y_test_predicted = np.sign(np.dot(data_package["x_test"], model))
    ...
    def get_accuracy(y, Y_predicted):
        ...
        correct_answers = (Y_predicted == y).astype(int)
        accuracy = float(sum(correct_answers)) / float(len(correct_answers))
        return accuracy, sum(correct_answers)

    train_score, correct_answers_train = get_accuracy(
        y_train, y_train_predicted)
    test_score, correct_answers_test = get_accuracy(
        data_package["y_test"], y_test_predicted)
...
def test_sensitive_attr_constraint_cov(
    ...
    ans = thresh - abs(cov)
    # ans = thresh - cov # will be <0 if the covariance is greater than thresh
    if verbose is True:
...
def print_covariance_sensitive_attrs(
        model, x_arr, y_arr_dist_boundary, x_control, sensitive_attrs):
    """
    ...
    for attr in sensitive_attrs:
        ...
        if not bin_attr:  # if its a non-binary sensitive feature, then perform one-hot-encoding
            attr_arr_transformed, index_dict = get_one_hot_encoding(attr_arr)

        thresh = 0

        if bin_attr:
            cov = thresh - test_sensitive_attr_constraint_cov(
                None, x_arr, arr, np.array(attr_arr), thresh, False)
            ...
            for attr_val, ind in index_dict.items():
                ...
                cov = thresh - \
                    test_sensitive_attr_constraint_cov(
                        None, x_arr, arr, t, thresh, False)
...
def get_correlations(
        model, x_test, y_predicted, x_control_test, sensitive_attrs):
    """
...
def get_constraint_list_cov(data_package, settings):
    """
    ...
    for attr in settings["sensitive_attrs"]:

        attr_arr = x_control_train[attr]
        attr_arr_transformed, index_dict = get_one_hot_encoding(attr_arr)

        if index_dict is None:  # binary attribute
            thresh = settings["sensitive_attrs_to_cov_thresh"][attr]
            c = ({'type': 'ineq', 'fun': test_sensitive_attr_constraint_cov, 'args': (
                x_train, y_train, attr_arr_transformed, thresh, False)})
            constraints.append(c)
            ...
            for attr_val, ind in index_dict.items():
                ...
                t = attr_arr_transformed[:, ind]
                c = (
                    {'type': 'ineq', 'fun': test_sensitive_attr_constraint_cov,
...
def get_avg_correlation_dict(correlation_dict_arr):
    ...
    for correlation_dict in correlation_dict_arr:
        for k, v in correlation_dict.items():
            for feature_val, feature_dict in v.items():
                for class_label, frac_class in feature_dict.items():
                    correlation_dict_avg[k][feature_val][class_label].append(
                        frac_class)
    ...
    for k, v in correlation_dict_avg.items():
        for feature_val, feature_dict in v.items():
            for class_label, frac_class_arr in feature_dict.items():
                correlation_dict_avg[k][feature_val][class_label] = np.mean(
                    frac_class_arr)

    return correlation_dict_avg


def plot_cov_thresh_vs_acc_pos_ratio(
        x_all, y_all, x_control_all, num_folds, loss_function,
        apply_fairness_constraints, apply_accuracy_constraint, sep_constraint,
        sensitive_attrs):

    # vary the covariance threshold using a range of decreasing multiplicative factors and
    # see the tradeoffs between accuracy and fairness
    it = 0.05
    ...
    test_acc = []
    # for each category (male / female), the frac of positive
    positive_per_category = defaultdict(list)

    # first get the original values of covariance in the unconstrained classifier --
    # these original values are not needed for reverse constraint
    test_acc_arr, _, correlation_dict_test_arr, _, _, cov_dict_train_arr = compute_cross_validation_error(
    ...
    for c in cov_range:
        ...
        sensitive_attrs_to_cov_original_multiplied = []
        for sensitive_attrs_to_cov_original in cov_dict_train_arr:
            sensitive_attrs_to_cov_thresh = deepcopy(
                sensitive_attrs_to_cov_original)
            ...
            sensitive_attrs_to_cov_original_multiplied.append(
                sensitive_attrs_to_cov_thresh)

        test_acc_arr, _, correlation_dict_test_arr, _, _, cov_dict_train_arr = compute_cross_validation_error(
            x_all, y_all, x_control_all, num_folds, loss_function, apply_fairness_constraints,
            apply_accuracy_constraint, sep_constraint, sensitive_attrs, sensitive_attrs_to_cov_original_multiplied, c)
        test_acc.append(np.mean(test_acc_arr))

        correlation_dict_test = get_avg_correlation_dict(
            correlation_dict_test_arr)

        # just plot the correlations for the first sensitive attr, the plotting can be extended for the other values,
        # but as a proof of concept, we will jsut show for one
        s = sensitive_attrs[0]
    ...
    p_rule_arr = (np.array(
        positive_per_category[0]) / np.array(positive_per_category[1])) * 100.0

    ax = plt.subplot(2, 1, 1)
    plt.plot(
        cov_range, positive_per_category[0],
        "-o", color="green", label="Protected")
    plt.plot(
        cov_range, positive_per_category[1],
        "-o", color="blue", label="Non-protected")
