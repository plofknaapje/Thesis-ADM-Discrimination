    ...
    def check_labels_bin(arr):
        ...
        try:
            if len(set(arr)) == 1:
                ...
                assert(sorted(list(set(arr)))[
                       0] == -1 and sorted(list(set(arr)))[1] == 1)
        except:
            traceback.print_exc()
            raise Exception(
                "Class labels (both true and predicted) can only take values -1 and 1... Exiting...")
...
def get_acc_stats(dist_dict, y, x_sensitive, verbose=False):
    ...
    try:
        assert(sorted(list(set(x_sensitive)))[
               0] == 0 and sorted(list(set(x_sensitive)))[1] == 1)
    except:
        raise Exception(
            "Sensitive feature can only take values 0 and 1... Exiting...")
    ...
    for s_val in set(x_sensitive):
        ...
        y_true_local = y[idx]
        # predictions with this classifier
        y_pred_local = sign_bin_clf(dist_dict[s_val][s_val])
        # predictions with other group's classifier
        y_pred_local_other = sign_bin_clf(dist_dict[s_val][other_val])

        assert(y_true_local.shape[0] == y_pred_local.shape[0]
               and y_true_local.shape[0] == y_pred_local_other.shape[0])

        acc_stats[s_val][s_val] = get_fp_fn_tp_tn(y_true_local, y_pred_local)
        acc_stats[s_val][other_val] = get_fp_fn_tp_tn(
            y_true_local, y_pred_local_other)
        ...
        if verbose:
            ...
            print("||  %s  || %0.2f (%0.2f) ||" % (
                s_val, acc_stats[s_val][s_val]["frac_pos"], acc_stats[s_val][other_val]["frac_pos"]))
...
def scale_data(x_train, x_test):
    ...
    for k in range(x_train.shape[1]):
        # checking if a column is binary
        idx_binary.append(np.array_equal(
            x_train[:, k], x_train[:, k].astype(bool)))
