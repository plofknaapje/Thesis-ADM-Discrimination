...
resamping_results <- all_metrics(resampled_df, model, df)
...
print("Unawareness")
weighted_unaware_model <- rpart(accepted ~ test_score + english_cert + extracurricular,
                        df, weights)
...
model <- decision_tree(mode = "classification")
massaging_results <- all_metrics(nB_df, model)
